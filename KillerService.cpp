...
void UndoFreezeTask() {
    ...
    CloseHandle(snap);
    if (resumed) {
        //std::wcout << L"[UndoFreeze] SeewoServiceAssistant.exe resumed.\n";
        {
            std::wstring logMsg = L"[UndoFreeze] SeewoServiceAssistant.exe resumed.";
            AppendLog(logMsg);
        }
    } else {
        //std::wcout << L"[UndoFreeze] No SeewoServiceAssistant.exe process found to resume.\n";
        {
            std::wstring logMsg = L"[UndoFreeze] No SeewoServiceAssistant.exe process found to resume.";
            AppendLog(logMsg);
        }
    }
    //std::wcout << L"[UndoFreeze] Task thread exiting.\n";
    {
...
void PipeWorker(HANDLE hPipe) {
    ...
    DWORD bytesRead = 0;
    static std::atomic<bool> killSeewoStarted{false};
    ...
    while (ReadFile(hPipe, buffer, sizeof(buffer) - sizeof(wchar_t), &bytesRead, NULL) && bytesRead > 0) {
        ...
        if (msg == L"KillSeewo") {
            ...
            {
                std::wstring logMsg = L"[UNKNOWN ACTION] Received unknown command: " + msg;
                AppendLog(logMsg);
            }
        }
